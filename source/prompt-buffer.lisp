...
(defun render-attributes (source prompt-buffer visible-sources-count)
    ...
    (when (prompter:suggestions source)
      (:table :class "source-content"
              ...
              (loop
                ...
                with max-suggestion-count = (if (> visible-sources-count 1) 8 12)
...
This does not redraw the whole prompt buffer, use `prompt-render' for that."
  (let* ((sources (prompter:sources prompt-buffer))
         ...
         (visible-sources-count (length (loop for i from current-source-index to last-source-index
                                              for source = (nth i sources)
                                              unless (null (prompter:suggestions source))
                                              collect source))))
    (flet ((source->html (source visible-sources-count)
             (spinneret:with-html-string
               (:div :class "source"
                     (:div :class "source-name"
                           ...
                           (if (prompter:ready-p source)
                               ""
                               "(In progress...)"))
                     (:raw (render-attributes source prompt-buffer visible-sources-count))))))
      (ps-eval :buffer prompt-buffer
        (setf (ps:@ (nyxt/ps:qs document "#suggestions") |innerHTML|)
              (ps:lisp
               (sera:string-join (loop for i from current-source-index to last-source-index
                                       for source = (nth i sources)
                                       unless (null (prompter:suggestions source))
                                         collect (source->html source visible-sources-count))
                                 +newline+)))))
